...
if __name__=="__main__":
  ...
  # Save disparity in OpenCV-readable format for C++
  disp_scaled = (disp * 256).astype(np.uint16)  # Scale and convert to 16-bit for better precision
  cv2.imwrite(f'{args.out_dir}/disparity.png', disp_scaled)
  
  # Also save as float32 TIFF for better precision if needed
  cv2.imwrite(f'{args.out_dir}/disparity.tiff', disp.astype(np.float32))
