...
# Delay cv2 import to avoid typing conflicts
# import cv2
import numpy as np
...
# Global cv2 variable to be imported when needed
cv2 = None

def _ensure_cv2():
    """Import cv2 when actually needed"""
    global cv2
    if cv2 is None:
        try:
            import cv2 as _cv2
...
def vis_disparity(disp, min_val=None, max_val=None, invalid_thres=np.inf, color_map=None, cmap=None, other_output={}):
  """
  @disp: np array (H,W)
  @invalid_thres: > thres is invalid
  """
  if not _ensure_cv2():
    print("Warning: OpenCV not available, returning grayscale disparity")
    disp_norm = ((disp - disp.min()) / (disp.max() - disp.min()) * 255).astype(np.uint8)
    return np.stack([disp_norm, disp_norm, disp_norm], axis=-1)
